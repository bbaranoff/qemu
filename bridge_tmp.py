        ...
        while running:
            ...
            if pty_fd in readable:
                ...
                for dlci, payload in parser.feed(raw):
                    ...
                    elif dlci == DLCI_DEBUG and len(payload) >= 8:
                        # DLCI 4 = burst data from BTS via PTY sercomm
                        tn = payload[0] & 0x07
                        fn = struct.unpack(">I", payload[1:5])[0]
                        soft_bits = payload[5:]
                        stats["burst"] = stats.get("burst", 0) + 1
                            ...
                            if block_id is not None and burst_idx is not None:
                                ...
                                if all(b is not None for b in burst_buf[block_id]):
                                    ...
                                    if l2 and len(l2) == 23:
                                        hdr = struct.pack("BBxx", MSG_DATA_IND, 0)
                                        info_dl = struct.pack(">BBHIBBBB",
                                            chan_nr, 0, 514, fn, 48, 20, 0, 0)
                                        msg = hdr + info_dl + l2
                                        frame = struct.pack(">H", len(msg)) + msg
                                        broadcast_raw(clients, frame)
                                        stats["decoded"] = stats.get("decoded", 0) + 1
                                        if stats["decoded"] <= 10 or stats["decoded"] % 100 == 0:
                                            print(f"  [burst→ALL] DATA_IND #{stats['decoded']} FN={fn} {l2.hex()}", flush=True)
