    ...
    case 0x4:
         ...
         *   0x4E-0x4F  DST src,Lmem             (mask 0xFE00) */
        {
            ...
            if (op8 == 0x4A) {
                ...
                /* DISP-ENTRY : trace PSHM ST0 (0x4A06) = save du ST0 ; on compare
                 * au POPM ST0 plus tard (même SP+val≠ = slot clobbé ; SP≠ = imbalance). */
                if (mmr == 0x06) {
                    g_last_pshm_st0_pc = s->pc; g_last_pshm_st0_sp = s->sp;
                    g_last_pshm_st0_val = val;
                }
                return consumed + s->lk_used;
            }
            if (op8 == 0x4B) {
