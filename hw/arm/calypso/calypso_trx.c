...
static void calypso_tdma_tick(void *opaque)
{
    ...
    /* MS-side: just tune frequencies, no POWERON/SETSLOT (BTS handles those).
     * POWERON is sent by run.sh via socat before QEMU starts. */
    if (!s->faketrx_powered && s->fn >= 300 && s->trxc_fd >= 0) {
        ...
        trxc_send_cmd(s, cmd);
        TRX_LOG("MS TUNE: ARFCN=%u DL=%u UL=%u (no POWERON)", arfcn, dl_khz, ul_khz);
