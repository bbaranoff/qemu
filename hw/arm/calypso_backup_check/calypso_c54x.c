...
static uint16_t prog_fetch(C54xState *s, uint16_t pc)
{
    ...
    if ((s->pmst & PMST_OVLY) && pc >= 0x80 && pc < 0x2800)
        return s->data[pc];
    /* For PC >= 0x8000: use XPC to select extended PROM page.
     * XPC=0: PROM0 (0x7000-0xDFFF), XPC=1: PROM1, XPC=2: PROM2, XPC=3: PROM3 */
    if (pc >= 0x8000 && s->xpc != 0) {
        uint32_t ext = ((uint32_t)s->xpc << 16) | pc;
...
{
    uint16_t addr16 = addr & 0xFFFF;
    /* OVLY: writes to 0x0080-0x27FF go to DARAM (data space) */
    if ((s->pmst & PMST_OVLY) && addr16 >= 0x80 && addr16 < 0x2800) {
        s->data[addr16] = val;
        return;
    }
    /* PROM0 (0x7000-0xFFFF with XPC=0) is ROM — ignore writes */
    if (addr16 >= 0x7000 && s->xpc == 0)
        return;
    /* Extended pages: allow write (could be RAM-backed on some configs) */
    if (addr16 >= 0x8000 && s->xpc != 0) {
        uint32_t ext = ((uint32_t)s->xpc << 16) | addr16;
        ext &= (C54X_PROG_SIZE - 1);
        s->prog[ext] = val;
        return;
    }
    /* Other program addresses (0x2800-0x6FFF): writable */
    s->prog[addr16] = val;
}
    ...
    if (opcode & 0x80) {
        ...
        *indirect = true;
        ...
        case 0xB: /* *ARn+0% */
            s->ar[cur_arp] += s->ar[0];
            break;
        case 0xC: /* *(lk) — absolute address in next word */
        {
            uint16_t lk = prog_fetch(s, s->pc + 1 + s->smem_extra);
            addr = lk;
            s->smem_extra++;
            break;
        }
        case 0xD: /* *+ARn(lk) — pre-increment with offset */
        {
            uint16_t lk = prog_fetch(s, s->pc + 1 + s->smem_extra);
        ...
        case 0xE: /* *ARn(lk) — indexed, no modify */
        {
            uint16_t lk = prog_fetch(s, s->pc + 1 + s->smem_extra);
            addr = s->ar[cur_arp] + lk;
            s->smem_extra++;
...
static int c54x_exec_one(C54xState *s)
{
    ...
    uint16_t addr;
    int consumed = 1;
    s->smem_extra = 0;  /* reset extra words consumed by Smem addressing */
    ...
    case 0xF:
        ...
        if (hi8 == 0xF0 || hi8 == 0xF1) {
            ...
            /* F073/F273: B[D] pmad — unconditional branch (2 words)
             * Per SPRU172C p.4-14: 1111 00Z0 0111 0011 + pmad
             * B = F073, BD = F273 */
            if (op == 0xF073 || op == 0xF273) {
                op2 = prog_fetch(s, s->pc + 1);
                consumed = 2;
                s->pc = op2;
                return 0;
            }
            /* F074/F274: CALL[D] pmad — unconditional call (2 words)
             * Per SPRU172C p.4-27: 1111 00Z0 0111 0100 + pmad
             * CALL = F074, CALLD = F274 */
            if (op == 0xF074 || op == 0xF274) {
                op2 = prog_fetch(s, s->pc + 1);
                consumed = 2;
                s->sp--;
                data_write(s, s->sp, (uint16_t)(s->pc + 2));
            ...
            /* F072/F272: RPTB[D] pmad (2 words) — block repeat
             * Per SPRU172C p.4-148: RSA=PC+2, REA=pmad, BRAF=1 */
            if ((op & 0xFDFF) == 0xF072) {
                op2 = prog_fetch(s, s->pc + 1);
                consumed = 2;
                s->rea = op2;
            ...
            /* F071/F271: RPT #lk (2 words) — repeat next instruction lk+1 times */
            if ((op & 0xFDFF) == 0xF071) {
                op2 = prog_fetch(s, s->pc + 1);
                s->rpt_count = op2;
                s->rpt_active = true;
            ...
            /* F0xx ALU/LD #lk,16 family (2-word):
             * F00x: LD  #lk,16,dst  — dst = lk << 16
             * F01x: AND #lk,16,dst  — dst = src AND (lk << 16)
             * F02x: OR  #lk,16,dst  — dst = src OR  (lk << 16)
             * F03x: XOR #lk,16,dst  — dst = src XOR (lk << 16)
             * Bits: 1111 0000 00TT SRC0, T=00=LD, T=01=AND, T=10=OR, T=11=XOR
             * Bit 2 (0x04): dst B (else A). Bit 0: src sel (0=same as dst). */
            if ((op & 0xFFC0) == 0xF000) {
                op2 = prog_fetch(s, s->pc + 1);
                consumed = 2;
                int dst_b = (op >> 2) & 1;
                int64_t *dst_acc = dst_b ? &s->b : &s->a;
                int src_b = (op & 1) ? !dst_b : dst_b;
                int64_t src_val = src_b ? s->b : s->a;
                int64_t lk16 = (int64_t)(uint32_t)op2 << 16;
                int alu_op = (op >> 4) & 3;
                switch (alu_op) {
                case 0: *dst_acc = sext40(lk16); break;           /* LD */
                case 1: *dst_acc = sext40(src_val & lk16); break; /* AND */
                case 2: *dst_acc = sext40(src_val | lk16); break; /* OR */
                case 3: *dst_acc = sext40(src_val ^ lk16); break; /* XOR */
                }
                return consumed;
            }
            /* F0xx (remaining): 2-word instruction, consume and advance. */
            {
                op2 = prog_fetch(s, s->pc + 1);
                consumed = 2;
                static int f0_unk = 0;
                if (f0_unk++ < 5)
                    C54_LOG("F0xx UNIMPL: 0x%04x 0x%04x PC=0x%04x", op, op2, s->pc);
                return consumed;
            }
        }
        ...
        if (hi8 == 0xF2 || hi8 == 0xF3) {
            ...
            s->t = xval_l;
            /* Post-modify */
            if (((op >> 4) & 0x1) == 0) s->ar[xar_l]++; else s->ar[xar_l]--;
        ...
        /* F800: RET — near return. Per SPRU172C p.4-139:
         * 1111 1000 0000 0000, (TOS)→PC, SP+1→SP */
        if (op == 0xF800) {
        ...
        if (hi8 == 0xF8) {
            ...
            /* F80x-F81x: RPT Smem — count from data memory */
        ...
        if (hi8 == 0xF8) {
            uint16_t old_arp_val = s->ar[arp(s)];
            addr = resolve_smem(s, op, &ind);
            op2 = prog_fetch(s, s->pc + 1);
            consumed = 2;
            if (old_arp_val != 0) {
        ...
        if (hi8 == 0xF5) {
            s->rpt_count = op & 0xFF;
            s->rpt_active = true;
            s->pc += 1;  /* single-word instruction */
            return 0;
        }
        ...
        if (hi8 == 0xF9) {
             ...
             * Advance PC past RPT+immediate NOW, set repeat for next insn. */
            op2 = prog_fetch(s, s->pc + 1);
            s->rpt_count = op2;
            s->rpt_active = true;
            s->pc += 2;  /* advance past RPT + immediate */
            return 0;    /* tell caller PC already set */
        }
        /* FAxx: RPTZ A, #k — Repeat and Zero A accumulator (2 words)
         * Per SPRU172C: 1111 1010 XXXX XXXX + 16-bit count
         * Zeros A accumulator, then repeats next instruction count+1 times */
        if (hi8 == 0xFA) {
            op2 = prog_fetch(s, s->pc + 1);
            consumed = 2;
            s->a = 0;
        ...
        /* FC00: RETD — delayed return. Per SPRU172C p.4-139:
         * 1111 1100 0000 0000, (TOS)→PC, SP+1→SP */
        if (op == 0xFC00) {
            uint16_t ra = data_read(s, s->sp); s->sp++;
            s->pc = ra; return 0;
        }
        /* FCxx: LD #k, 16, B (for k != 0) */
        if (hi8 == 0xFC) {
            int8_t k = (int8_t)(op & 0xFF);
            int64_t v = (int64_t)k << 16;
            s->b = sext40(v);
    ...
    case 0xE:
        ...
        if (hi8 == 0xEC || hi8 == 0xED) {
            /* EC/ED: BC/BCD pmad, cond (conditional branch, 2 words)
             * Per SPRU172C condition encoding:
             *   Bits 3:0 + bit 4: accumulator condition
             *   Bit 5: C flag, Bit 6: TC flag
             *   Bit 7: complement (NOT) for group2 (C/TC) conditions
             * All set condition bits must be true (AND logic). */
            op2 = prog_fetch(s, s->pc + 1);
            consumed = 2;
            uint8_t cond = op & 0xFF;
            bool take = true;

            /* Group 1: accumulator conditions (bits 3:0) */
            if (cond & 0x0F) {
                int64_t acc = (cond & 0x08) ? s->b : s->a;
                bool acc_cond = false;
                switch (cond & 0x07) {
                case 0x0: acc_cond = true; break;           /* UNC */
                case 0x1: acc_cond = (acc >= 0); break;     /* GEQ */
                case 0x2: acc_cond = (acc != 0); break;     /* NEQ */
                case 0x3: acc_cond = (acc == 0); break;     /* EQ */
                case 0x4: acc_cond = (acc > 0); break;      /* GT */
                case 0x5: acc_cond = (acc <= 0); break;     /* LEQ */
                case 0x6: acc_cond = false; break;          /* OV (not tracked) */
                case 0x7: acc_cond = true; break;           /* NOV (not tracked) */
                }
                take = take && acc_cond;
            }
            /* Group 2: TC flag (bit 6) */
            if (cond & 0x40) {
                bool tc = (s->st0 & ST0_TC) != 0;
                if (cond & 0x80) tc = !tc;  /* complement */
                take = take && tc;
            }
            /* Group 2: C flag (bit 5) */
            if (cond & 0x20) {
                bool c = (s->st0 & ST0_C) != 0;
                if (cond & 0x80) c = !c;  /* complement */
                take = take && c;
            }
            /* No group bits set = unconditional */
            if ((cond & 0x6F) == 0) take = true;

            if (op2 == 0x7E92) {
                static int bc_trace = 0;
                if (bc_trace < 20) {
                    C54_LOG("BC 0x7E92: cond=0x%02x take=%d B=0x%010llx A=0x%010llx TC=%d C=%d insn=%u",
                            cond, take, (unsigned long long)(s->b & 0xFFFFFFFFFFLL),
                            (unsigned long long)(s->a & 0xFFFFFFFFFFLL),
                            (s->st0 & ST0_TC) ? 1 : 0, (s->st0 & ST0_C) ? 1 : 0,
                            s->insn_count);
                    bc_trace++;
                }
            }
        ...
        if (hi8 == 0xEE) {
             ...
             * Same condition encoding as EC (BC): group1 AND group2. */
            op2 = prog_fetch(s, s->pc + 1);
            consumed = 2;
            uint8_t cond = op & 0xFF;
            bool take = true;
            ...
            if (cond & 0x0F) {
                ...
                bool acc_cond = false;
                switch (cond & 0x07) {
                case 0x0: acc_cond = true; break;
                case 0x1: acc_cond = (acc >= 0); break;
                case 0x2: acc_cond = (acc != 0); break;
                case 0x3: acc_cond = (acc == 0); break;
                case 0x4: acc_cond = (acc > 0); break;
                case 0x5: acc_cond = (acc <= 0); break;
                case 0x6: acc_cond = false; break;
                case 0x7: acc_cond = true; break;
                }
                take = take && acc_cond;
            }
            if (cond & 0x40) {
        ...
        /* 0xED handled above with 0xEC (same condition logic) */
        if (hi8 == 0xE8) {
            /* E8xx: CMPR cond, ARn */
            int cmp_cond = (op >> 4) & 3;
            int n = arp(s);
    ...
    case 0x6: case 0x7:
        ...
        /* 74xx/75xx: MVDP Smem, pmad — Move Data to Program (2 words)
         * Per SPRU172C: writes data[Smem] to prog[pmad] */
        if (hi8 == 0x74 || hi8 == 0x75) {
            addr = resolve_smem(s, op, &ind);
            op2 = prog_fetch(s, s->pc + 1);
            consumed = 2;
        ...
        /* 78xx-7Bxx: BANZ[D] pmad, Sind — Branch if AR not zero (2 words)
         * Per SPRU172C p.4-16: 0111 1Z0I AAAA AAAA + pmad
         * BANZ = 78xx/79xx, BANZD = 7Axx/7Bxx */
        if (hi8 >= 0x78 && hi8 <= 0x7B) {
            addr = resolve_smem(s, op, &ind);
            op2 = prog_fetch(s, s->pc + 1);
            consumed = 2;
            uint16_t ar_val = s->ar[arp(s)];
            if (ar_val != 0) {
                s->pc = op2;
                return 0;
            }
        ...
        /* 7Cxx/7Dxx: READA Smem — Read program memory addressed by A
         * Per SPRU172C p.4-136: 0111 110I AAAA AAAA
         * A → PAR, Pmem[PAR] → Smem. PAR auto-increments on repeat. */
        ...
        if ((op & 0xF800) == 0x7800) {
            /* 7Exx-7Fxx: STH src, Smem (remaining range after BANZ/READA) */
            int src_acc = (op >> 9) & 1;
            addr = resolve_smem(s, op, &ind);
            int64_t acc = src_acc ? s->b : s->a;
    ...
    case 0x8: case 0x9:
        ...
        if (hi8 == 0x90 || hi8 == 0x91 || hi8 == 0x92 || hi8 == 0x93) {
            ...
            if (((op >> 4) & 0x1) == 0) s->ar[xar_m]++; else s->ar[xar_m]--;
        ...
        if ((op & 0xFC00) == 0x9C00) {
            ...
            /* Xmem post-modify */
            if (((op >> 4) & 0x1) == 0) s->ar[xar_s]++; else s->ar[xar_s]--;
            return consumed;
        }
        if (hi8 == 0x8A) {
            /* MVDK Smem, dmad */
    ...
    case 0xA: case 0xB:
        ...
        if (hi8 == 0xA4 || hi8 == 0xA5 || hi8 == 0xA6 || hi8 == 0xA7 ||
            ...
            /* Post-modify */
            if (((op >> 4) & 0x1) == 0) s->ar[xar_d]++; else s->ar[xar_d]--;
        ...
        if (hi8 == 0xA1) {
            ...
            if (((op >> 4) & 0x1) == 0) s->ar[xar_sq]++; else s->ar[xar_sq]--;
        ...
        if (hi8 == 0xBC || hi8 == 0xBD || hi8 == 0xBE || hi8 == 0xBF) {
            ...
            if (((op >> 4) & 0x1) == 0) s->ar[xar_p]++; else s->ar[xar_p]--;
        ...
        if (hi8 == 0xB8 || hi8 == 0xB9 || hi8 == 0xBA || hi8 == 0xBB) {
            ...
            if (((op >> 4) & 0x1) == 0) s->ar[xar_b8]++; else s->ar[xar_b8]--;
...
ba_handler:
        if (hi8 == 0xAA) {
            /* STLM src, MMR — store low accumulator to MMR */
            int src_acc = (op >> 4) & 1;
            uint16_t mmr = op & 0x1F;
            int64_t acc = src_acc ? s->b : s->a;
        ...
        if (hi8 == 0xAB) {
            /* STHM src, MMR — store high accumulator to MMR
             * Per SPRU172C: Smem = src << (ASM-16), MMR addressed */
            int src_acc = (op >> 4) & 1;
            uint16_t mmr = op & 0x1F;
            int64_t acc = src_acc ? s->b : s->a;
            data_write(s, mmr, (uint16_t)((acc >> 16) & 0xFFFF));
            return consumed;
        }
        if (hi8 == 0xBD) {
    ...
    case 0xC: case 0xD:
        ...
        if (hi8 >= 0xD0 && hi8 <= 0xD9 && hi8 != 0xDA) {
            ...
            if (((op >> 4) & 0x1) == 0) s->ar[xar_c]++; else s->ar[xar_c]--;
        ...
        if (hi8 == 0xCA || hi8 == 0xCB) {
            ...
            if (((op >> 4) & 0x1) == 0) s->ar[xar_ca]++; else s->ar[xar_ca]--;
        ...
        if (hi8 == 0xC8 || hi8 == 0xC9) {
            ...
            if (((op >> 4) & 0x1) == 0) s->ar[xar]++; else s->ar[xar]--;
            if ((op & 0x08) == 0) s->ar[yar]++; else s->ar[yar]--;
            return consumed;
        }
        goto unimpl;
...
int c54x_run(C54xState *s, int n_insns)
{
    ...
    while (executed < n_insns && s->running && !s->idle) {
        ...
        /* Trap: SP changes drastically */
        {
            static uint16_t prev_sp = 0;
            static int sp_log = 0;
            if (s->sp != prev_sp && sp_log < 20) {
                C54_LOG("SP change 0x%04x → 0x%04x PC=0x%04x insn=%u op=0x%04x",
                        prev_sp, s->sp, s->pc, s->insn_count, prog_fetch(s, s->pc));
                sp_log++;
                prev_sp = s->sp;
            }
        }
        /* Trap: PC=0 means crash — dump trail */
        if (s->pc == 0 && s->insn_count > 100) {
            static int pc0_log = 0;
            if (pc0_log < 3) {
                C54_LOG("PC=0 CRASH! insn=%u SP=0x%04x PMST=0x%04x trail:",
                        s->insn_count, s->sp, s->pmst);
                for (int i = 20; i > 0; i--)
                    C54_LOG("  -%02d: PC=0x%04x", i, pc_ring[(pc_ring_idx - i) & 255]);
                pc0_log++;
            }
        }
        ...
        if (run_num <= 2 && executed < 2000) {
            C54_LOG("BOOT[%d.%d] PC=0x%04x op=0x%04x SP=0x%04x ST0=0x%04x A=0x%010llx B=0x%010llx",
                    run_num, executed, s->pc, prog_fetch(s, s->pc), s->sp, s->st0,
                    (unsigned long long)(s->a & 0xFFFFFFFFFFLL),
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        while (sscanf(p, " %hx%n", &word, (int[]){0}) == 1) {
            ...
            if (section == 0) {
                ...
                /* PDROM is also the internal boot ROM content, accessible
                 * in both data space (via DROM bit) and program space.
                 * Copy to prog[0xE000-0xFFFF] for interrupt vectors + boot code. */
                if (section == 2) {
                    uint16_t paddr = addr & 0xFFFF;
                    if (paddr >= 0xE000)
                        s->prog[paddr] = word;
                }
            } else {
                ...
                /* PROM1 NOT mirrored to 16-bit space 0xE000-0xFFFF:
                 * that range contains the boot ROM (from PDROM section).
                 * PROM1 is accessible via XPC=1 at prog[0x18000+]. */
            }
            addr++;
            total_words++;
        }
