...
typedef struct CalypsoTRX {
    ...
    QEMUTimer   *tint0;          /* single TDMA frame timer — the Calypso TINT0 */
    uint32_t     fn;
    bool         tdma_running;
...
static uint64_t calypso_dsp_read(void *opaque, hwaddr offset, unsigned size)
{
    ...
    /* DSP boot: bootloader version handshake.
     * ARM reads DL_STATUS until it sees BOOT status, then proceeds. */
...
static void calypso_dsp_write(void *opaque, hwaddr offset, uint64_t value, unsigned size)
{
    ...
    /* DSP bootloader protocol emulation.
     * The real bootloader lives in internal program ROM (not in our dump).
     * From PROM0 at 0xB400, we know it sets ST0=0x1800 (TC=1,C=1),
     * SP=0x5AC8, then CALLs the target address.
     * ARM writes commands to API RAM, we emulate the bootloader response. */
    if (offset == 0x0FFE) {
        uint16_t cmd = (uint16_t)value;
        if (cmd == 0 || cmd == 2 || cmd == 4) {
            TRX_LOG("DSP bootloader: cmd=%d → IDLE", cmd);
            s->dsp_ram[0x0FFE/2] = 1;  /* BL_STATUS_IDLE */
        }
        if (cmd == 2 && s->dsp) {
            uint16_t bl_size = s->dsp_ram[0x0FFA/2];
            uint16_t bl_addr = s->dsp_ram[0x0FFC/2];
            uint16_t bl_addr_hi = s->dsp_ram[0x0FF8/2];
            if (bl_size == 0 && bl_addr != 0) {
                /* JUMP: emulate what bootloader ROM does before calling target.
                 * From PROM0 B401-B403: STM #0x1800,ST0; STM #0x5AC8,SP
                 * Then CALL target → push return addr at data[SP], SP--. */
                TRX_LOG("DSP bootloader: JUMP to 0x%04x", bl_addr);
                c54x_reset(s->dsp);
                /* Bootloader register setup (from PROM0 B400-B40F) */
                s->dsp->st0 = 0x1800;   /* TC=1, C=1 (from STM #0x1800,ST0 at B401) */
                s->dsp->sp  = 0x5AC8;   /* from STM #0x5AC8,SP at B403 */
                /* The init code (B360) ends with RETE which pops data[0x5AC8].
                 * Set it to our IDLE stub at prog[0xFFFE].
                 * On each frame, calypso_trx sets PC to dispatch entry. */
                s->dsp->data[s->dsp->sp] = 0xFFFE;
                /* Write IDLE loop stub: IDLE + B self */
                s->dsp->prog[0xFFFE] = 0xF4E4; /* IDLE */
                s->dsp->prog[0xFFFF] = 0xF880; /* B 0xFFFE */
                s->dsp->prog[0x0000] = 0xFFFE; /* (target of B) */

                /* Bootloader ROM copies (from PDROM FF00-FF4E):
                 * 1. MVPD prog[0xFF7D-0xFF7F] → data[0x0080-0x0082] (3 words)
                 * 2. Zero DARAM ranges 0x0080-0x27FF (except the 3 words above)
                 * These copies prepare DARAM for OVLY program execution. */
                s->dsp->data[0x0080] = 0xC60F;
                s->dsp->data[0x0081] = 0x7CE7;
                s->dsp->data[0x0082] = 0x004D;
                s->dsp->sp--;
                s->dsp->pc = bl_addr;
                s->dsp->running = true;
                s->dsp_booted = true;
                s->dsp_ram[0x01A8/2] = 0;
                TRX_LOG("DSP boot: PC=0x%04x SP=0x%04x ST0=0x%04x data[0x5AC8]=0x%04x (dispatch)",
                        bl_addr, s->dsp->sp, s->dsp->st0, s->dsp->data[0x5AC8]);
            } else if (bl_size > 0) {
                TRX_LOG("DSP bootloader: COPY %d words → DSP 0x%04x:%04x",
                        bl_size, bl_addr_hi, bl_addr);
                for (int i = 0; i < bl_size && i < 0x2000; i++) {
                    uint16_t w = s->dsp_ram[i];
                    s->dsp->data[bl_addr + i] = w;
                }
            }
        }
    }
...
/* ---- TPU ---- */
static void calypso_tint0_start(CalypsoTRX *s);
...
static void calypso_tpu_write(void *o, hwaddr off, uint64_t val, unsigned sz) {
    ...
    if (off==TPU_INT_CTRL && !(val&ICTRL_MCU_FRAME) && !s->tdma_running) calypso_tint0_start(s);
...
static void calypso_tint0_tick(void *opaque) {
    CalypsoTRX *s = opaque;
    ...
    if (s->dsp && s->tpu_en_pending) {
        ...
        /* Bridge: set DSP to dispatch loop for this frame.
         * The dispatch loop is a chain of subroutines in PROM0.
         * We enter at the frame setup (C8CF) and push IDLE stub
         * as return address so DSP returns to IDLE when done. */
        s->dsp->sp = 0x5AC8;
        s->dsp->data[0x5AC8] = 0xFFFE;  /* return to IDLE stub */
        s->dsp->pc = 0xC8CF;            /* dispatch: after CALL C8E7 */
        s->dsp->idle = false;
        dsp_should_run = true;
    }
    ...
    if (dsp_should_run && s->dsp) {
        ...
        /* Run DSP until IDLE.
         * Init phase (IMR=0): heavy MVPD. Use 5M/tick (keeps VM responsive).
         * Normal frames (IMR set): ~461K insn per 4.615ms. Use 500K. */
        int budget = s->dsp->imr ? 500000 : 5000000;
        int ran = 0, chunk;
        ...
        /* Sync d_dsp_page back from DSP → ARM after dispatch.
         * The DSP writes the new page value to api_ram[0xD4] during
         * its dispatch loop; propagate it back to dsp_ram so the ARM
         * side sees the updated page on next frame. */
        if (s->dsp->idle && s->dsp->api_ram) {
            s->dsp_ram[0x01A8/2] = s->dsp->api_ram[0x08D4 - C54X_API_BASE];
        }
    ...
    /* Re-arm + kick CPU to process pending IRQs */
    if (s->tdma_running)
        timer_mod_ns(s->tint0, qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL) + GSM_TDMA_NS);
...
static void calypso_tint0_start(CalypsoTRX *s)
{
    ...
    s->tdma_running = true;
    s->fn = 0;
    TRX_LOG("TINT0 started (master clock @ %.3f ms)", GSM_TDMA_NS / 1e6);
    timer_mod_ns(s->tint0, qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL) + GSM_TDMA_NS);
}
...
bool calypso_trx_burst_ready(void)
{
    ...
    if (!s->dsp_booted || !s->tdma_running || !l1ctl_burst_mode())
        return false;
...
/* RX burst from bridge (DL) — sync FN only.
 * Burst data flows through PTY/UART → ARM L1 → DSP dispatch.
 * No direct injection into DSP RAM. */
...
void calypso_trx_init(MemoryRegion *sysmem, qemu_irq *irqs)
{
    ...
    s->dsp_ram[DSP_DL_STATUS_ADDR/2] = 0; s->dsp_booted = false;
    ...
    /* Single master clock — Calypso TINT0 at TDMA frame rate */
    s->tint0 = timer_new_ns(QEMU_CLOCK_VIRTUAL, calypso_tint0_tick, s);
