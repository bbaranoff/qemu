...
static void l1ctl_accept_cb(void *opaque)
{
    ...
    /* Don't register read handler yet — wait for firmware to speak first.
     * This is client/server: firmware (server) talks first, mobile (client) responds. */
    L1CTL_LOG("client connected (fd=%d) — waiting for firmware", fd);
    ...
    if (!runstate_is_running()) {
        L1CTL_LOG("resuming VM — client ready");
        vm_start();
    }
}
