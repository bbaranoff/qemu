...
HACK="/opt/GSM/qemu-src/hack_gdb.py"
...
for k in $(seq 1 "$NRUNS"); do
    ...
    CALYPSO_BSP_DARAM_ADDR=0x3f80 \
    CALYPSO_BSP_DARAM_LEN=128 \
    CALYPSO_DBG=all \
    ...
    # hack.py disabled — FBSB orchestration is now in calypso_fbsb.c.
    # The GDB BP injection is incompatible with the new path: it forces
    # r8/r3 in the firmware, which breaks the legitimate ARM↔DSP NDB
    # protocol that calypso_fbsb.c is observing/driving.
